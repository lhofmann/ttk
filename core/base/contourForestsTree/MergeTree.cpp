...
#pragma omp critical
  ...
  // It can't be more connected component that leaves so test for each leaves
  // (even virtual extrema)
  for(const auto &l : treeData_.leaves) {
    ...
    if(treeData_.superArcs[tmp_sa].getUpNodeId() == nullNodes) {
      ...
      Node *originNode
        = getNode(getNode(getSuperArc(tmp_sa)->getDownNodeId())->getOrigin());
...
void MergeTree::processVertex(const SimplexId &currentVertex,
  ...
  if(!neighSize) {
    ...
    for(auto *neigh : vect_neighUF) {
      closeSuperArc(
        (idSuperArc)neigh->find()->getData(), closingNode, overlapB, overlapA);
        ...
        for(idSuperArc ua = 0; ua < nbup; ua++) {
          ...
          if(arcDownNode != nid || !arc.isVisible()) {
            ...
            if(upnode == nullNodes || downnode == nullNodes) {
              ...
              cout << " Problem with up arc : "
                   << printArc(node.getUpSuperArcId(ua)) << endl;
            }
          }
        }
        ...
        for(idSuperArc da = 0; da < nbdown; da++) {
          ...
          if(arcUpNode != nid || !arc.isVisible()) {
            ...
            if(upnode == nullNodes || downnode == nullNodes) {
              ...
              cout << "[Verif] Node " << node.getVertexId() << " id : " << nid;
              cout << " Problem with down arc : "
                   << printArc(node.getUpSuperArcId(da)) << endl;
            }
          }
        }
      }
